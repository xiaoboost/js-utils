...
import { isFunc, isDef, isUndef, isArray } from './assert';
...
/** 连接数组 */
export function concat<T, U>(from: T[], cb: (val: T) => (U | undefined)[] | U | undefined): U[] {
  let result: U[] = [];
...
export function toMap<T extends AnyObject, U extends Index, V>(
  arr: T[],
  toKey: IndexCb<T, U>,
  toVal: IndexCb<T, V>,
): Record<U, V | undefined>;
export function toMap<T extends AnyObject, U extends Index, V>(
  arr: T[],
  toKey: IndexCb<T, U>,
  toVal?: IndexCb<T, V>,
...
export function exclude<T extends AnyObject, U extends Index>(
  arr: T[],
  rest: T[],
  cb: IndexCb<T, U>,
): T[];
